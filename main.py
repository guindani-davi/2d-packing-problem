...
class Bin:
    def __init__(self, width, height):
        ...
        self.rectangles = []
        self.used_area = 0
        self.occupancy_map = [[False for _ in range(width)] for _ in range(height)]
    ...
    def find_first_fit_position(self, rect):
        # Encontra a primeira posição válida para o retângulo (FFD)
...
def read_input_file(filename):
    ...
    # Linhas seguintes: retângulos (width height)
    rectangles = []
...
def first_fit_decreasing_grasp(bin_width, bin_height, rectangles, alpha, max_bins=100):
...
def grasp_construction(bin_width, bin_height, rectangles, alpha, max_bins=100):
    ...
    for _ in range(10):  # Número de iterações GRASP
        ...
        solution = first_fit_decreasing_grasp(bin_width, bin_height, randomized_rects, alpha, max_bins)
...
def main():
    ...
    # Escrever solução em arquivo
    with open("solucao_ffd_grasp.txt", 'w') as f:
        for i, bin in enumerate(solution, 1):
